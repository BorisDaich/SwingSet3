...
public class ColorBlind {
	...
	public static final String[] COLORBLIND_TYPE = 
		...
		, "Deuteranopia"  // green-blindness, ~ 5% of man
	 ...
	 * 
	 * @param color     Java Color object containing values for RGB
	 * @param colorType PROTANOPIA = 1, DEUTERANOPIA = 2 or TRITANOPIA = 3 as
	 *                  declared above
	 * @return Dichromat Color
	 */
	...
	public static Color getDichromatColor(Color color, int colorType) {
		...
		double reddiff = XYZtoRGBMatrix[0][0] * (Xn - X) + XYZtoRGBMatrix[0][1] * (Yn - Y)
				+ XYZtoRGBMatrix[0][2] * (Zn - Z);
		double greendiff = XYZtoRGBMatrix[1][0] * (Xn - X) + XYZtoRGBMatrix[1][1] * (Yn - Y)
				+ XYZtoRGBMatrix[1][2] * (Zn - Z);
		double bluediff = XYZtoRGBMatrix[2][0] * (Xn - X) + XYZtoRGBMatrix[2][1] * (Yn - Y)
		...
		// System.out.println("Red: " + red + " Green: " + green + " Blue: " + blue);
		// System.out.println("Red: " + red * 255.0 + " Green: " + green * 255.0 + "
		// Blue: " + blue * 255.0);

		Color retValue = new Color((float) red, (float) green, (float) blue);
		log.fine("Color " + color.getRed() + "-" + color.getGreen() + "-" + color.getBlue() + " -> " + retValue.getRed()
				+ "-" + retValue.getGreen() + "-" + retValue.getBlue() + " <- " + COLORBLIND_TYPE[colorType]);
