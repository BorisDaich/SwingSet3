...
/*
 * generiert aus feather pause (non solid) und hierhin kopiert + organize imports
 ...
 */
public class PauseIcon implements RadianceIcon {
...
shape = new Rectangle2D.Double(6.0, 4.0, 4.0, 16.0);
g.setPaint(paint);
g.setStroke(stroke);
g.draw(shape);
...
paint = (colorFilter != null) ? colorFilter.filter(new Color(0, 0, 0, 255)) : new Color(0, 0, 0, 255);
stroke = new BasicStroke(2.0f,1,1,4.0f,null,0.0f);
shape = new Rectangle2D.Double(14.0, 4.0, 4.0, 16.0);
g.setPaint(paint);
g.setStroke(stroke);
g.draw(shape);
g.setTransform(transformsStack.pop());
